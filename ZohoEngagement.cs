...
namespace ZohoEngagement
{
    public class ZohoEngagement
    {
        ...
        public ZohoEngagement(ILoggerFactory loggerFactory, HttpClient httpClient, IConfiguration configuration, ApplicationDbContext context)
        {
            _logger = loggerFactory.CreateLogger<ZohoEngagement>();
        ...
        [Function("ZohoCasesByDays")]
        public async Task RunZohoCasesByDaysAsync([TimerTrigger("0 0 * * 0")] TimerInfo myTimer)
        {
            _logger.LogInformation($"C# Timer trigger function executed at: {DateTime.Now}");
            await GetZohoCasesByDaysAsync();
        }

        [Function("ZohoDeliverablesPublished")]
        public async Task RunDeliverablesPublishedAsync([TimerTrigger("0 0 * * 0")] TimerInfo myTimer)
        {
            _logger.LogInformation($"C# Timer trigger function executed at: {DateTime.Now}");
            await GetDeliverablesPublishedAsync();
        }

        [Function("ZohoDeliverablesDueDate")]
        public async Task RunDeliverablesDueDateAsync([TimerTrigger("0 0 * * 0")] TimerInfo myTimer)
        {
            _logger.LogInformation($"C# Timer trigger function executed at: {DateTime.Now}");
            await GetDeliverablesDueDateAsync();
        }
        ...
        public async Task<string> GetZohoCasesByDaysAsync()
        {
            ...
            else
            {
                try
                {
                    ...
                    DateTime endOfPreviousMonth = DateTime.Today;
